...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nazva)) {
        ...
        // Додаємо новий вид кави
        $kava_id = addKava($nazva, $opis, $tsina, $chas_prihotuvannya, $dostupna);
        
        if ($kava_id) {
            // Додаємо інгредієнти, якщо вони вибрані
            if (isset($_POST['ingredients']) && is_array($_POST['ingredients'])) {
                foreach ($_POST['ingredients'] as $ingredient_id => $kilkist) {
                    if (is_numeric($kilkist) && $kilkist > 0) {
                        addIngredientToKava($kava_id, $ingredient_id, $kilkist);
                    }
                }
            }
            
            $message = 'Новий вид кави успішно додано!';
            
            header("Location: view_kava.php?id=$kava_id&success=1");
            exit;
        } else {
            $error = 'Помилка при додаванні кави';
        }
    }
}
?>
...
<head>
    ...
    <title>Додати новий вид кави</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>Додати новий вид кави</h1>
        
        <p><a href="index.php" class="btn btn-secondary">← Повернутися до списку</a></p>
        ...
        <form method="post" action="">
            ...
            <div class="form-group">
                ...
                <input type="number" id="chas_prihotuvannya" name="chas_prihotuvannya" min="1" required value="<?php echo isset($_POST['chas_prihotuvannya']) ? htmlspecialchars($_POST['chas_prihotuvannya']) : ''; ?>">
            </div>
            
            <div class="form-group">
                <label>
                    <input type="checkbox" name="dostupna" value="1" <?php echo (!isset($_POST['dostupna']) || $_POST['dostupna']) ? 'checked' : ''; ?>>
                    ...
                    <?php if (count($ingredienty) > 0): ?>
                        <?php foreach ($ingredienty as $ingredient): ?>
                            <div class="ingredient-item">
                                ...
                                <input type="number" name="ingredients[<?php echo $ingredient['id']; ?>]" min="0" step="0.01" value="0">
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?>
            ...
            <div class="form-group">
                <button type="submit" class="btn btn-primary">Додати каву</button>
            </div>
        </form>
    </div>
</body>
